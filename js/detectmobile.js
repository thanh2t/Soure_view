...
function DetectAndroidhai()
{
	if (uagent.search(android22) > -1)
        return true;
     //Yay! It's an iPhone!
     else 
        return false;
...
function detectMobile(btn) {
	  var host = location.protocol + '//' + location.host;
	  
	  if(DetectIphoneAndAndroid()) {
		  $(btn).attr({href:host + "/mobile_usr_entry/"});
	  } else {
		  $(btn).attr({href:host + "/usr_entry/"});
	  }
	  
}
function detectDevice(btn) {
	if(DetectIphone()){ //Device is Iphone
		$(btn).height("60px");
		$(btn).css({"margin-bottom":"7%"});
	}
	else if(DetectAndroidPhone) { //Device is Android Phone
		$(btn).height("100px");
...
//**************************
//Detects if the current device is an iPhone.
function DetectIphoneAndAndroid()
{
if (DetectAndroidPhone() || DetectIphone())
{
	return true;
}
else
